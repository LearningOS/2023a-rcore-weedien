...
pub fn sys_thread_create(entry: usize, arg: usize) -> isize {
    ...
    // 创建新线程时，需要初始化need数组，表示线程可能会占用的锁资源
    // 其数量继承自主线程（0号线程）
    process_inner.finished.push(false);

    // let mutex_num = process_inner.mutex_need.get(0).map_or(0, |v| v.len());
    let semaphore_num = process_inner.semaphore_need.get(0).map_or(0, |v| v.len());
